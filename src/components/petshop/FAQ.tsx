  ...
  return (
    <section className="py-16 md:py-20 px-4" style={{ background: "#fafaf9" }}>
      <div className="max-w-[800px] mx-auto">
        ...
        <h2 className="text-2xl md:text-[2.4rem] font-black mb-12 leading-tight" style={{ fontFamily: "'Fraunces', Georgia, serif", color: "var(--txt)" }}>
          ...
          {faqs.map((faq, i) => {
            ...
            return (
              <div
                ...
                style={{ border: `1px solid ${isOpen ? "var(--verde)" : "var(--borda)"}` }}
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : i)}
                  className="w-full flex items-center justify-between p-5 text-left"
                >
                  <span className="text-[15px] font-semibold pr-4" style={{ color: "var(--txt)" }}>
                    {faq.q}
                  </span>
                  <ChevronDown
                    className="w-5 h-5 flex-shrink-0 transition-transform duration-200"
                    style={{
                      color: isOpen ? "var(--verde)" : "var(--txt2)",
                ...
                >
                  <p className="px-5 pb-5 text-sm leading-relaxed" style={{ color: "var(--txt2)" }}>
                    {faq.a}
                  </p>
                </div>
              </div>
