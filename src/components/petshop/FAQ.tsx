  ...
  return (
    <section className="py-16 md:py-20 px-4 bg-stone-50" ref={ref}>
      <div className={`max-w-[800px] mx-auto transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <h2 className="text-2xl md:text-[2rem] font-black mb-8" style={{ fontFamily: "'Fraunces', Georgia, serif", color: "#1c1917" }}>
        ...
        <div className="space-y-2">
          {faqs.map((faq, i) => {
            ...
            return (
              <div key={i} className="rounded-xl bg-white overflow-hidden transition-all duration-200 border border-stone-200">
                <button
                  onClick={() => setOpenIndex(isOpen ? null : i)}
                  className="w-full flex items-center justify-between p-5 text-left group"
                >
                  <span className="text-sm font-semibold pr-4" style={{ color: "#1c1917" }}>{faq.q}</span>
                  <ChevronDown
                    className="w-4 h-4 flex-shrink-0 transition-transform duration-200"
                    style={{ color: "#a8a29e", transform: isOpen ? "rotate(180deg)" : "rotate(0)" }}
                  />
                </button>
                <div className="overflow-hidden transition-all duration-300" style={{ maxHeight: isOpen ? 200 : 0, opacity: isOpen ? 1 : 0 }}>
                  <p className="px-5 pb-5 text-sm leading-relaxed" style={{ color: "#78716c" }}>{faq.a}</p>
                </div>
              </div>
            );
          })}
