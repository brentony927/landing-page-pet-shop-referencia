...
const Contact = () => {
  ...
  return (
    <section id="contato" className="py-16 md:py-20 px-4" style={{ background: "var(--pet-bg-alt)" }} ref={ref}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-start">
          <div className="space-y-3">
            {[
              ...
              >
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "var(--pet-green-light)" }}>
                  <c.icon className="w-4 h-4" style={{ color: "var(--pet-green)" }} />
                </div>
                <div>
                  <h4 className="text-sm font-extrabold" style={{ color: "#333" }}>{c.emoji} {c.title}</h4>
                  {c.isLink ? (
                    <a href={c.href} className="text-[13px] font-bold" style={{ color: "var(--pet-green)" }}>{c.info}</a>
                  ) : (
                    <p className="text-[13px]" style={{ color: "var(--pet-gray)" }}>{c.info}</p>
                  )}
                  {c.link && (
                    <a href={c.link.href} target="_blank" rel="noopener noreferrer" className="text-[13px] font-bold block" style={{ color: "var(--pet-green)" }}>{c.link.text}</a>
                  )}
                </div>
              </div>
            ))}

            <div className="rounded-2xl overflow-hidden mt-4" style={{ border: "1.5px solid var(--pet-gray-border)" }}>
              <iframe
          ...
          <form onSubmit={handleSubmit} className="rounded-2xl p-8 bg-white" style={{ border: "1.5px solid var(--pet-gray-border)", boxShadow: "0 4px 24px rgba(0,0,0,0.04)" }}>
            <h3 className="text-lg font-extrabold mb-6" style={{ fontFamily: "'Baloo 2', cursive", color: "#333" }}>
              🐾 Agendar serviço
            </h3>
            ...
            {[
              ...
              <div key={field.key} className={`mb-3 ${field.half ? "inline-block w-[calc(50%-6px)] mr-3 align-top" : "block"}`}>
                ...
                <input
                  ...
                  onFocus={(e) => { e.currentTarget.style.borderColor = "var(--pet-green)"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(76,175,80,0.12)"; }}
