...
const Contact = () => {
  ...
  return (
    <section id="contato" className="py-16 md:py-20 px-4" style={{ background: "var(--cream-dark)" }} ref={ref}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-start">
          <div className="space-y-3">
            {[
              ...
              >
                <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: "var(--forest-light)" }}>
                  <c.icon className="w-4 h-4" style={{ color: "var(--forest)" }} />
                </div>
                <div>
                  <h4 className="text-sm font-semibold" style={{ color: "var(--txt-primary)" }}>{c.title}</h4>
                  {c.isLink ? (
                    <a href={c.href} className="text-[13px] font-bold" style={{ color: "var(--forest)" }}>{c.info}</a>
                  ) : (
                    <p className="text-[13px]" style={{ color: "var(--txt-secondary)" }}>{c.info}</p>
                  )}
                  {c.link && (
                    <a href={c.link.href} target="_blank" rel="noopener noreferrer" className="text-[13px] font-semibold block" style={{ color: "var(--forest)" }}>{c.link.text}</a>
                  )}
                </div>
              </div>
            ))}
          ...
          >
            <h3 className="text-lg font-bold mb-6" style={{ fontFamily: "'Playfair Display', Georgia, serif", color: "var(--txt-primary)" }}>
              Agendar serviço
            </h3>
            ...
            {[
              ...
              <div key={field.key} className={`mb-3 ${field.half ? "inline-block w-[calc(50%-6px)] mr-3 align-top" : "block"}`}>
                <label className="block text-[13px] font-semibold mb-1.5" style={{ color: "var(--txt-secondary)" }}>{field.label}</label>
                <input
                  className="w-full border rounded-lg px-3.5 py-2.5 text-sm outline-none transition-all duration-200"
                  style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--txt-primary)", borderColor: "var(--border)" }}
                  placeholder={field.placeholder}
                  value={form[field.key as keyof typeof form]}
                  onChange={(e) => setForm({ ...form, [field.key]: e.target.value })}
                  required={field.key !== "pet"}
                  onFocus={(e) => { e.currentTarget.style.borderColor = "var(--forest)"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(13,51,33,0.08)"; }}
            ...
            <div className="mb-4">
              <label className="block text-[13px] font-semibold mb-1.5" style={{ color: "var(--txt-secondary)" }}>Serviço</label>
              <select
                className="w-full border rounded-lg px-3.5 py-2.5 text-sm outline-none transition-all duration-200 appearance-auto"
                style={{ fontFamily: "'DM Sans', sans-serif", color: "var(--txt-primary)", borderColor: "var(--border)" }}
                value={form.servico}
