    ...
    <section id="contato" className="py-12 sm:py-16 lg:py-20 px-4" style={{ background: "#F8FAFD" }} ref={ref}>
      <div className={`max-w-[1100px] mx-auto scroll-fade-up ${visible ? "visible" : ""}`}>
        <div className="text-center mb-8">
          <span className="inline-block text-[10px] font-extrabold uppercase tracking-[0.15em] mb-2 px-4 py-1.5 rounded-full" style={{ background: "#FFF3E8", color: "#F5851F" }}>Contato</span>
          <h2 className="text-[22px] sm:text-3xl lg:text-4xl font-extrabold" style={{ fontFamily: "'Baloo 2', cursive", color: "#1a3a5c" }}>Fale com a gente</h2>
        </div>
            ...
            {infos.map(c => (
              <div key={c.title} className="flex items-center gap-3 rounded-2xl p-4 bg-white hover:shadow-md transition-all" style={{ border: "1.5px solid #f0f0f0" }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "#E3F2FD" }}>
                  <c.icon className="w-4 h-4" style={{ color: "#1E88E5" }} />
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-extrabold" style={{ color: "#1a3a5c" }}>{c.title}</p>
                  {c.href ? (
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full rounded-xl px-3.5 py-2.5 text-sm outline-none transition-all focus:ring-2 focus:ring-blue-100 bg-gray-50" style={{ border: "1.5px solid #eee", color: "#333" }} placeholder="Seu nome" value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} required />
                </div>
                <div>
                  <label className="block text-[11px] font-bold mb-1" style={{ color: "#888" }}>Telefone</label>
                  <input className="w-full rounded-xl px-3.5 py-2.5 text-sm outline-none transition-all focus:ring-2 focus:ring-blue-100 bg-gray-50" style={{ border: "1.5px solid #eee", color: "#333" }} placeholder="(69) 99999-0000" value={form.telefone} onChange={e => setForm({ ...form, telefone: e.target.value })} required />
                </div>
              </div>
              <div>
                <label className="block text-[11px] font-bold mb-1" style={{ color: "#888" }}>Nome do pet</label>
                <input className="w-full rounded-xl px-3.5 py-2.5 text-sm outline-none transition-all focus:ring-2 focus:ring-blue-100 bg-gray-50" style={{ border: "1.5px solid #eee", color: "#333" }} placeholder="Como chama o bichinho?" value={form.pet} onChange={e => setForm({ ...form, pet: e.target.value })} />
              </div>
              <div>
                <label className="block text-[11px] font-bold mb-1" style={{ color: "#888" }}>Serviço</label>
                <select className="w-full rounded-xl px-3.5 py-2.5 text-sm outline-none transition-all focus:ring-2 focus:ring-blue-100 bg-gray-50 appearance-auto" style={{ border: "1.5px solid #eee", color: "#333" }} value={form.servico} onChange={e => setForm({ ...form, servico: e.target.value })}>
