...
import { Clock, Phone, MapPin, Send, Shield, CheckCircle, Circle, Star, Dog, Cat, PawPrint } from "lucide-react";
import { useInView } from "@/hooks/useInView";
import { useOpenStatus } from "@/hooks/useOpenStatus";

const portes = ["Pequeno", "Médio", "Grande"];
const tiposPet = ["Cachorro", "Gato", "Outro"];
...
const Contact = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const msg = [
      ...
      `🐶 *Pet:* ${form.pet} (${form.tipoPet}, porte ${form.porte})`,
      `✂️ *Serviço:* ${form.servico}`,
            ...
            {infos.map(c => (
              <div key={c.title} className="flex items-center gap-3.5 rounded-2xl p-4 bg-white hover:shadow-md transition-all duration-300 hover:-translate-y-0.5" style={{ border: "1.5px solid #f0f0f0" }}>
                ...
                <div className="min-w-0">
                  ...
                  {c.href ? (
                    <a href={c.href} target="_blank" rel="noopener noreferrer" className="text-sm font-bold" style={{ color: "var(--pet-action)" }}>{c.info}</a>
                  ) : (
            ...
            <div className="space-y-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input className="w-full rounded-xl px-3.5 py-2.5 text-sm outline-none transition-all duration-200 focus:ring-2 bg-gray-50/80" style={{ border: "1.5px solid #eee", color: "#333", "--tw-ring-color": "var(--pet-orange-light)" } as React.CSSProperties} placeholder="Ex: Thor" value={form.pet} onChange={e => setForm({ ...form, pet: e.target.value })} required />
                </div>
                <div>
                  <label className="block text-[11px] font-bold mb-1.5" style={{ color: "#888" }}>Tipo</label>
                  <div className="flex gap-1.5">
                    {tiposPet.map(t => (
                      <button key={t} type="button" onClick={() => setForm({ ...form, tipoPet: t })} className="flex-1 rounded-lg py-2 text-[11px] font-bold transition-all" style={{ background: form.tipoPet === t ? "var(--pet-action)" : "#f5f5f5", color: form.tipoPet === t ? "#fff" : "#888", border: form.tipoPet === t ? "1.5px solid var(--pet-action)" : "1.5px solid #eee" }}>
                        {t === "Cachorro" ? "🐶" : t === "Gato" ? "🐱" : "🐾"} {t}
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold mb-1.5" style={{ color: "#888" }}>Porte</label>
                  <div className="flex gap-1.5">
                    {portes.map(p => (
                      <button key={p} type="button" onClick={() => setForm({ ...form, porte: p })} className="flex-1 rounded-lg py-2 text-[11px] font-bold transition-all" style={{ background: form.porte === p ? "var(--pet-action)" : "#f5f5f5", color: form.porte === p ? "#fff" : "#888", border: form.porte === p ? "1.5px solid var(--pet-action)" : "1.5px solid #eee" }}>
                        {p === "Pequeno" ? "P" : p === "Médio" ? "M" : "G"}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <label className="block text-[11px] font-bold mb-1.5" style={{ color: "#888" }}>Observação (opcional)</label>
                <textarea className="w-full rounded-xl px-3.5 py-2.5 text-sm outline-none transition-all duration-200 focus:ring-2 bg-gray-50/80 resize-none" rows={2} style={{ border: "1.5px solid #eee", color: "#333", "--tw-ring-color": "var(--pet-orange-light)" } as React.CSSProperties} placeholder="Ex: Meu pet tem medo de secador" value={form.observacao} onChange={e => setForm({ ...form, observacao: e.target.value })} />
              </div>
            </div>
