  ...
  return (
    <section className="py-16 md:py-20 px-4" style={{ background: "var(--pet-blue-light)" }} ref={ref}>
      <div className={`max-w-[1100px] mx-auto transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <h2 className="text-2xl md:text-[2rem] font-extrabold mb-10" style={{ fontFamily: "'Baloo 2', cursive", color: "var(--pet-blue-dark)" }}>
          💬 Quem já veio, voltou
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {testimonials.map((t, idx) => (
            <div
              ...
              style={{
                background: idx === 1 ? "var(--pet-blue)" : "white",
                border: idx === 1 ? "none" : "1.5px solid var(--pet-blue)",
                boxShadow: "0 8px 32px rgba(66,165,245,0.08)",
              }}
              onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = idx === 1 ? "0 16px 40px rgba(66,165,245,0.3)" : "0 12px 30px rgba(66,165,245,0.12)"; }}
            ...
            >
              <div className="flex gap-0.5 mb-4">
                {Array.from({ length: 5 }, (_, i) => (
                  <span key={i} className="text-base" style={{ color: "var(--pet-orange)" }}>★</span>
                ))}
              </div>
              ...
              <div className="flex items-center gap-3">
                <img src={t.petImage} alt={`Pet ${t.petName}`} className="w-10 h-10 rounded-full object-cover" style={{ border: idx === 1 ? "2px solid rgba(255,255,255,0.4)" : "2px solid var(--pet-blue)" }} />
                <div>
                  ...
                  <p className="text-[11px] font-bold" style={{ color: "var(--pet-orange)" }}>{t.petName}</p>
                </div>
              </div>
            </div>
          ))}
