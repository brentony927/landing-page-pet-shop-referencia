    ...
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      >
        <div className="max-w-[1200px] mx-auto flex items-center justify-between h-full px-4 lg:px-6">
          <a href="#inicio" className="flex items-center gap-2">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center" style={{ background: "var(--pet-orange)" }}>
              <PawPrint className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
            </div>
            <span className="text-lg sm:text-xl font-extrabold" style={{ fontFamily: "'Baloo 2', cursive", color: "var(--pet-blue-dark)" }}>
              Demonstração
            </span>
          </a>
          ...
          <button className="md:hidden p-2 -mr-2" style={{ color: "var(--pet-gray)" }} onClick={() => setMobileOpen(!mobileOpen)}>
      ...
      {mobileOpen && (
        <>
          ...
          <div
            className="fixed inset-y-0 right-0 z-50 w-[85%] max-w-[320px] md:hidden flex flex-col bg-white"
            style={{ animation: "slideInRight 0.3s ease" }}
          >
            <div className="flex items-center justify-between px-5 py-4" style={{ background: "var(--pet-orange)" }}>
              <span className="text-lg font-extrabold text-white" style={{ fontFamily: "'Baloo 2', cursive" }}>🐾 Menu</span>
              <button onClick={() => setMobileOpen(false)} className="p-1 text-white/80"><X className="w-5 h-5" /></button>
            </div>
            <nav className="flex-1 px-5 py-3">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="flex items-center text-[15px] font-bold py-4 transition-colors duration-200"
                  style={{ color: "var(--pet-gray)", borderBottom: "1px solid var(--pet-gray-border)" }}
            ...
            <div className="px-5 py-5 space-y-3" style={{ background: "var(--pet-bg-alt)" }}>
              <a href="https://wa.me/5569992216764" className="block text-center rounded-xl py-3.5 text-sm font-extrabold tracking-wider text-white" style={{ background: "var(--pet-blue)", boxShadow: "0 4px 14px rgba(30,136,229,0.3)" }}>
                💬 WHATSAPP
              </a>
              <a href="https://wa.me/5569992216764" className="block text-center rounded-xl py-3.5 text-sm font-extrabold tracking-wider text-white" style={{ background: "var(--pet-orange)", boxShadow: "0 4px 14px rgba(245,133,31,0.3)" }}>
                🐕 AGENDAR AGORA
              </a>
            </div>
          </div>
